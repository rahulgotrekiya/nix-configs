  ...
  services.grafana = {
    ...
    settings = {
      server = {
        http_addr = "0.0.0.0";
        http_port = 3000;
        domain = "monitor.gotrekiya.site";
      };
      
      security = {
        admin_user = "admin";
        admin_password = "$__file{${config.sops.secrets."grafana/admin_password".path}}";
        # nixpkgs 26.05+ requires an explicit secret_key (no default). Reuse the
        # sops admin_password file so nothing lands in the repo.
        # ponytail: reuses admin_password as the DB secret key; add a dedicated
        # sops `grafana/secret_key` if DB-stored secrets ever need separate rotation.
        secret_key = "$__file{${config.sops.secrets."grafana/admin_password".path}}";
      };
