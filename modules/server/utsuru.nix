...

{
  ...
  services.aria2 = {
    enable = true;
    openFirewall = true;
    # RPC secret (optional, but good for security)
    rpcSecretFile = null; # Add one here if you want: config.sops.secrets."aria2/rpc_secret".path
    settings = {
      enable-rpc = true;
      rpc-listen-all = true;
  ...
  virtualisation.oci-containers.containers.utsuru = {
    ...
    environment = {
      ...
      PORT = "4096";
      HOST = "0.0.0.0";
      # Use host IP since aria2 runs on the host network
      ARIA2_RPC_URL = "http://172.17.0.1:6800/jsonrpc"; # Docker bridge IP
      JELLYFIN_URL = "http://172.17.0.1:8096";
      DATABASE_PATH = "/app/data/utsuru.db";
      MEDIA_MOVIES_DIR = "/media/movies";
      MEDIA_TV_DIR = "/media/tv";
      # Change these! (Or use SOPS)
